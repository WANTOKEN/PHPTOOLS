...
class PHPExcelTools
{
    ...
    public function exportBrowser($fileName = "Excel", $headerArr = array(), $data = array(), $sheet = 'Sheet1')
    {
        $this->excelCopy = 'Excel2007';
        $this->suffix = '.xlsx';
        $this->objPHPExcel->setActiveSheetIndex(0); //设置当前的sheet
        $objSheet = $this->objPHPExcel->getActiveSheet(); //获取当前活动sheet
        $objSheet->setTitle($sheet); //设置标题
        ...
        $headCharArr = $this->getHeaderChar($headerArr); //A B C
        foreach ($headCharArr as $k => $v) {
            $this->objPHPExcel->setActiveSheetIndex(0)->getStyle($headCharArr[$k] . '1')->getFont()->setBold(true); //设置当前的sheet
            $objSheet = $this->objPHPExcel->getActiveSheet(); //获取当前活动sheet
            $objStyle = $objSheet->getStyle($headCharArr[$k] . '1');
            $objAlign = $objStyle->getAlignment();
            $objAlign->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER); //上下居中
            $objAlign->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER); //左对齐
            $objSheet->setCellValue($headCharArr[$k] . '1', $headerArr[$k]);
        }
    ...
    function exportCSV($fileName, $headerArr, $dataArr)
    {
        ...
        foreach ($dataArr as $item) {
            $rows = array();
            foreach ($item as $k=>$exportObj) {
                $rows[$k] = iconv('utf-8', 'utf-8', $exportObj);
            }
            fputcsv($fp, $rows);
        }
        fclose($fp);
